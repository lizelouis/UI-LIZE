...
vu32 STM32_FLASH_ReadWord(u32 faddr)
{
	return *(vu32*)faddr; 
} 
...
void STM32_FLASH_Write(u32 WriteAddr,u16 *pBuffer,u16 NumToWrite)	
{
	...
	{	
		...
		if(i<secremain)//需要擦除
		{
			FLASH_EraseSector(secpos*STM32_SECTOR_SIZE+STM32_FLASH_BASE,VoltageRange_3);//擦除这个扇区
...
void STM32_FLASH_Read32(u32 ReadAddr,u32 *pBuffer,u32 NumToRead)   	
{
	...
	for(i=0;i<NumToRead;i++)
	{
		pBuffer[i]=STM32_FLASH_ReadWord(ReadAddr);//读取2个字节.
		ReadAddr+=4;//偏移2个字节.	
	}
}
